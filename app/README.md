...
설치형 EXE와 Portable EXE는 `release/0.0.0/`에 생성됩니다.
...
같은 Wi-Fi의 Android 앱이 PC를 자동으로 찾아 연결합니다. 비밀키 입력 없이 근무·휴가·교육·일정·반복업무를 질문하고, PC 월 이동과 일정 등록·변경·삭제를 할 수 있습니다. 변경은 휴대폰 미리보기 확인 후 실행합니다. 응답은 휴대폰 화면과 한국어 음성으로 제공합니다.

Android 0.2.1은 태블릿 가로 화면에 맞춰 대기·연결과 질문·답변을 나누어 표시합니다. ‘까치야, 내일 야간 누구야?’처럼 이어서 말할 수 있으며 PC에는 변환된 텍스트만 보냅니다. 화면이 꺼져도 실행 중인 마이크 서비스가 대기하며, 입력이 끊기면 자동 복구를 시도합니다. 첫 실행에서 권한을 허용하고 음성 준비를 기다려 주세요. 기기 재부팅·강제 종료 후에는 앱을 다시 열어야 합니다.
