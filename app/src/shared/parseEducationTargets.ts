...
export function serializeEducationTargets(targets: string[], description: string): string {
  const names = [...new Set(targets.map((name) => name.trim()).filter(Boolean))]
  return [names.length ? `${EDUCATION_TARGET_PREFIX}${names.join(', ')}` : '', description].filter(Boolean).join('\n')
}
