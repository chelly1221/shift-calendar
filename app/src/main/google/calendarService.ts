...
import { buildShiftNameContext, inferEventMetadata, toGoogleSummary, buildShiftGoogleSummary, type ShiftNameContext } from '../../shared/eventTitleMapper'
...
/** 제목 기반 휴가 추론용 팀원 이름·약어 컨텍스트 */
export type RemoteSnapshotOptions = ShiftNameContext
...
export function toRemoteSnapshot(
  ...
  const inferred = inferEventMetadata(rawSummary, rawDescription, rawEventType, options)
...
/** 제목 기반 휴가 추론용 팀원 이름·약어 — 설정 조회 실패 시 빈 컨텍스트(추론은 휴리스틱만으로 진행) */
async function loadShiftNameContext(): Promise<ShiftNameContext> {
  try {
    return buildShiftNameContext(await getShiftSettings())
  } catch (error) {
    console.warn('[CalendarService] failed to load shift settings for title inference', error)
    return {}
  }
}
...
export function createGoogleCalendarService(): GoogleCalendarService {
  ...
  return {
    ...
    async pullRemoteEvents(query) {
      ...
      const nameContext = await loadShiftNameContext()

      const events = (response.data.items ?? [])
        .map((item) => {
          const snapshot = toRemoteSnapshot(item, nameContext)
          if (!snapshot) {
